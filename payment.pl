...
function validateForm ( form )
  ...
  if (option1 == "Select account to make payment" )
     { alert("You didn't select a account!");
        return false;
         }
  if (form.to.value == "" )
     { alert("You didn't supply receiver's account!");
        return false;
         }
...
<form action="/cgi-bin/Team1/paymentDB.pl" onSubmit="return validateForm(this)">
<PRE>
<b>Make A Payment

Select the Accounts to be used for the Payment, enter the payment Amount. 
Payment Instructions</b>
<table width="100%" cellspacing=0>
<tr height="30"><td width="1%" >
         <b>From: </b></td> <td  >
             <select name="From" id="frm">
                 <option>Select account to make payment
                  <option>Checking(#$ch)
                  <option>Saving(#$sa)
             </select>
         </td></tr>
...
<b>Receiver's account number</b></label><input type="text" name="to" id="to" size="30">
