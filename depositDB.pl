       ...
       {  while (@account1=$sth->fetchrow()) {
          ...
          #close statement handle
            ...
            if ( $fromacc eq "Saving(#".$sa[0].")" ) {
              ...
              $sth = $dbh->prepare($update1);
              $sth->execute($newbal1, $userid) or print "Can't execute!";
              $sth->finish();
       my $d=do gdate();
    my $insert1="INSERT INTO transaction (accnum, trandate, trantype, debit, credit, balance) VALUES (?,?, 'ECheck Deposit#$to',0 ,?, ?)";
         my $sth1 = $dbh->prepare($insert1);
             $sth1->execute($fr, $d, $amt,  $newbal1) or print "Can't execute!";
             $sth1->finish();

print<<EF;
<p>Your deposit is complete. Thank you!<a href=\"/cgi-bin/Team1/main.pl\">Click here</a> to go to main page.
</body></html>
EF
